// Admin panel — pedidos, produtos, pagamentos, galeria, perfis, agenda. Login com usuário e senha.
...
const ADMIN_SESSION = 'aa-admin-session';
...
function Admin({open, onClose, showToast}) {
  ...
  const acc = store.settings.account;
  const [authed, setAuthed] = adUseState(() => { try { return sessionStorage.getItem(ADMIN_SESSION) === '1'; } catch(e){ return false; } });
  const [user, setUser] = adUseState('');
  const [pass, setPass] = adUseState('');
  const [err, setErr] = adUseState(false);
  ...
  const login = (e) => {
    e.preventDefault();
    const ok = user.trim().toLowerCase() === (acc.user||'').toLowerCase() && pass === acc.pass;
    if (ok) { setAuthed(true); setErr(false); try { sessionStorage.setItem(ADMIN_SESSION,'1'); } catch(x){} }
    else { setErr(true); setPass(''); }
  };
  const logout = () => { setAuthed(false); try { sessionStorage.removeItem(ADMIN_SESSION); } catch(x){} onClose(); };
    ...
    return (
      <div className="admin-root">
        <div className="admin-login">
          ...
          <p style={{color:'var(--ink-500)',marginBottom:'1.8rem',fontSize:'.95rem'}}>Entre com seu usuário e senha.</p>
          <form onSubmit={login} style={{display:'grid',gap:'.9rem',textAlign:'left'}}>
            <Field label="Usuário"><input value={user} onChange={e=>{setUser(e.target.value);setErr(false)}} autoComplete="username" placeholder="angella" autoFocus/></Field>
            <Field label="Senha"><input type="password" value={pass} onChange={e=>{setPass(e.target.value);setErr(false)}} autoComplete="current-password" placeholder="••••••••"/></Field>
            {err && <div className="admin-err">Usuário ou senha incorretos.</div>}
            <button className="btn btn-primary" style={{width:'100%',marginTop:'.4rem'}} type="submit">Entrar <Icon.Arrow size={14}/></button>
          </form>
          <div className="admin-hint">Acesso inicial: <strong>{acc.user}</strong> / <strong>{acc.pass}</strong> — troque em Pagamentos → Acesso.</div>
        </div>
      </div>
    );
  }
...
function Pagamentos({store, showToast}) {
  ...
  const [pass2, setPass2] = adUseState('');
    ...
    <>
      ...
      <div className="pay-grid">
        ...
        <div className="admin-panel">
          ...
          <p style={{color:'var(--ink-500)',fontSize:'.86rem',margin:'.4rem 0 1.2rem'}}>Troque a senha inicial antes de publicar o site.</p>
          <div className="form-grid">
            <Field label="Nome"><input value={cfg.account.name} onChange={e=>upd('account',{name:e.target.value})}/></Field>
            <Field label="Usuário"><input value={cfg.account.user} onChange={e=>upd('account',{user:e.target.value})}/></Field>
            <Field label="Nova senha"><input type="password" value={cfg.account.pass} onChange={e=>upd('account',{pass:e.target.value})}/></Field>
            <Field label="Confirmar senha"><input type="password" value={pass2} onChange={e=>setPass2(e.target.value)}/></Field>
          </div>
          <button className="btn btn-primary" style={{marginTop:'1rem'}} onClick={()=>{
            if (cfg.account.pass.length < 6) return showToast('Use ao menos 6 caracteres');
            if (pass2 !== cfg.account.pass) return showToast('As senhas não conferem');
            showToast('Acesso atualizado');
          }}>Salvar acesso <Icon.Check size={14}/></button>
        </div>
      </div>
    </>
  );
...
function Galeria({gallery, setGallery, showToast}) {
  ...
  const addFiles = async (files) => {
    ...
    try {
      ...
      for (const f of list) {
        const media = await uploadMedia(f);
        const data = await adminFetch('/gallery', {method:'POST', body: JSON.stringify({
