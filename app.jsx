...
const SERVICES_DATA = [
  ...
  {n:'02', title:'Massoterapia',      icon:'Leaf',     tags:['Relaxante','Drenagem','Modeladora','Pedras Quentes'],
    body:'Toque terapêutico para corpo e mente. Drenagem linfática, massagem relaxante e protocolos de bem-estar em ambiente acolhedor.'},
...
function App() {
  ...
  const scrollTo = (id) => {
    setMenuOpen(false);
    if (page !== 'studio') { goPage('studio'); setTimeout(() => scrollTo(id), 320); return; }
    ...
    <>
      <nav className={`nav ${page==='studio'?'over-hero':''} ${scrolled?'scrolled':''}`}>
        <div className="nav-inner">
          ...
          <div className="nav-links">
            <a onClick={(e)=>{e.preventDefault();scrollTo('servicos')}}>Serviços</a>
            <a onClick={(e)=>{e.preventDefault();scrollTo('galeria')}}>Galeria</a>
            <a onClick={(e)=>{e.preventDefault();scrollTo('instagram')}}>Instagram</a>
            <a onClick={(e)=>{e.preventDefault();scrollTo('loja')}}>Loja</a>
            <a className={page==='massoterapia'?'on':''} onClick={(e)=>{e.preventDefault();goPage('massoterapia')}}>Massoterapia</a>
            <a onClick={(e)=>{e.preventDefault();scrollTo('agendar')}}>Agendar</a>
          </div>
          <div className="nav-cart">
            ...
            <button className="menu-toggle" onClick={()=>setMenuOpen(true)} aria-label="Menu"><Icon.Menu/></button>
          </div>
        </div>
      </nav>
      ...
      <div className={`mobile-menu ${menuOpen?'open':''}`}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'2rem'}}>
          ...
          <button onClick={()=>setMenuOpen(false)} style={{width:42,height:42}}><Icon.Close/></button>
        </div>
        <a onClick={()=>scrollTo('servicos')}>Serviços</a>
        <a onClick={()=>scrollTo('galeria')}>Galeria</a>
        <a onClick={()=>scrollTo('instagram')}>Instagram</a>
        <a onClick={()=>scrollTo('loja')}>Loja</a>
        <a onClick={()=>goPage('massoterapia')}>Massoterapia</a>
        <a onClick={()=>scrollTo('agendar')}>Agendar</a>
      </div>
        ...
        <div className="container">
          <div className="foot-grid">
            ...
            <div>
              <h4>Studio</h4>
              <ul>
                <li><a onClick={()=>goPage('massoterapia')}>Massoterapia</a></li>
                <li><a onClick={()=>scrollTo('servicos')}>Serviços</a></li>
                <li><a onClick={()=>scrollTo('galeria')}>Galeria</a></li>
                <li><a href={store.instagram?.url} target="_blank" rel="noopener">Instagram</a></li>
                <li><a onClick={()=>scrollTo('loja')}>Loja</a></li>
              </ul>
            </div>
            <div>
              <h4>Cliente</h4>
              <ul>
                <li><a onClick={()=>scrollTo('agendar')}>Agendar</a></li>
